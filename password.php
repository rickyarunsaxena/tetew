...
?>
	<div class="container">
		<div class="content">
			<h2>Data Pasien R.A.S &raquo; Ganti Password</h2>
			<hr />
			
			<p>Ganti password Pasien dengan No.Pasien <?php echo '<b>'.$_GET['nomor_kartu'].'</b>'; // mengambil nilai nomor kartu dari data yang dipilih ?></p>
			<?php
			if(isset($_POST['ganti'])){ // jika tombol 'Simpan' dengan properti name="ganti" pada baris 64 ditekan
				...
				$cek = mysqli_query($koneksi, "SELECT * FROM pasien WHERE nomor_kartu='$nomor_kartu' AND password='$password'"); // query memilih nomor_kartu dan password
				if(mysqli_num_rows($cek) == 0){ // mengecek query $cek jika password yang dimasukkan tidak sesuai dengan nomor_kartu
					...
					if($password1 == $password2){ // jika nilai password1 dan password2 bernilai sama
						if(strlen($password1) >= 6){ // mengecek panjang password minimal 6 karakter
							...
							$update = mysqli_query($koneksi, "UPDATE pasien SET password='$pass' WHERE nomor_kartu='$nomor_kartu'"); // query update password dari nomor_kartu yang dipilih
