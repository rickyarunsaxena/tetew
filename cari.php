...
?>
	<div class="container">
		<div class="content">
			<?php $nomor_kartu = $_POST['carinomor_kartu']; // mengambil nomor_kartu dari form cari ?> 
			<h2>Pencarian Data Pasien &raquo; Nomor Kartu: <?php echo $nomor_kartu; // menampilkan nomor_kartu ?></h2>
			<hr />
			
			<?php
			$sql = mysqli_query($koneksi, "SELECT * FROM pasien WHERE nomor_kartu='$nomor_kartu'"); // query untuk memilih entri dengan nomor_kartu terpilih
			...
			?>
			<!-- bagian ini digunakan untuk menampilkan data pasien hasil pencarian-->
			...
			<a href="edit.php?nomor_kartu=<?php echo $row['nomor_kartu']; ?>" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Edit Data</a>
			<a href="profile.php?aksi=delete&nomor_kartu=<?php echo $row['nomor_kartu']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Anda yakin akan mengahapus data <?php echo $row['nama']; ?>')"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Hapus Data</a>
		</div> <!-- /.content -->
	</div> <!-- /.container -->
<?php 
